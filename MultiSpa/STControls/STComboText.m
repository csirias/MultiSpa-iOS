...
- (void)showOptions {
	if(!self.viewOfPicker) {
        ...
        [self.tBar setItems:[NSArray arrayWithObjects:itemFlex,itemDone, nil] animated:YES];
    ...
    if(is_ipad())
    {
        if(UIInterfaceOrientationIsLandscape([self.delegateVCtr interfaceOrientation])){
            self.viewOfPicker.frame=CGRectMake(0, 0, 271, 240);
            self.pkrView.frame=CGRectMake(0, 44, 271, 216);
            self.tBar.frame=CGRectMake(0, 0, 271, 44);
        } else {
            self.viewOfPicker.frame=CGRectMake(0, 0, 271, 240);
            self.pkrView.frame=CGRectMake(0, 44, 271, 216);
            self.tBar.frame=CGRectMake(0, 0, 271, 44);
	...
	action=[[UIActionSheet alloc] initWithTitle:@"sagar" delegate:self cancelButtonTitle:@"asdf" destructiveButtonTitle:@"adsf" otherButtonTitles:@"adsf",@"asodk", nil];
